...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["name"] == "") {
        $prevImagePathQuery = "SELECT img_path FROM challenge WHERE ch_id = $id";
        $result = mysqli_query($conn, $prevImagePathQuery);
        if ($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $prevImagePath = $row['img_path'];

            if (!empty($prevImagePath) && file_exists($target_dir . $prevImagePath)) {
                unlink($target_dir . $prevImagePath);
            }
        }

        $sql = "UPDATE challenge SET sem = $sem, year = '$year', challenge = '$challenge', 
            plan = '$plan', remark = '$remark', img_path = NULL WHERE ch_id = $id";
        $status = update_DBTable($conn, $sql);

        if ($status) {
            echo '<script type="text/javascript">
                alert("Challenge record updated successfully.");
        ...
        if($uploadOk){
            ...
            if ($status) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    ...
                    echo '<script type="text/javascript">
                        alert("Challenge record with image file is successfully updated!");
                        window.location.replace("my_challenge.php");
                    </script>';    
                } 
                else{
